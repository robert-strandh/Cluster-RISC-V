  ...
  (ecase (ldb (byte 7 0) instruction)
    (#b0000011
     (let* ((offset (ldb (byte 12 20) instruction))
            ...
            (base-register-number (ldb (byte 5 16) instruction))
            (base-register (integer-register base-register))
            (destination-register-number (ldb (byte 5 7) instruction))
            (destination-register
              (integer-register destination-register-number)))
    ...
    (#b0100011
     (let* ((offset (logior (ldb (byte 7 25) instruction)
            ...
            (width (ldb (byte 3 12) instruction))
            (source-register-number (ldb (byte 5 20) instruction))
            (source-register (integer-register source-register-number))
            (base-register-number (ldb (byte 5 16) instruction))
            (base-register (integer-register base-register-number)))
    ...
    (#b0110011
     (let* ((func-7 (ldb (byte 7 25) instruction))
            ...
            (source-register-1-number (ldb (byte 5 16) instruction))
            (source-register-1
              (integer-register source-register-1-number))
            (source-register-2-number (ldb (byte 5 20) instruction))
            (source-register-2
              (integer-register source-register-2-number))
            (destination-register-number (ldb (byte 5 7) instruction))
            (destination-register
              (integer-register destination-register-number)))
       (disassemble-integer-register-register-instruction
        func-7 func-3
        source-register-1 source-register-2 destination-register)))
    (#b0010011
     (let* ((raw-immediate-value (ldb (byte 12 20) instruction))
            (func-3 (ldb (byte 3 12) instruction))
            (source-register-number (ldb (byte 5 16) instruction))
            (source-register (integer-register source-register-number))
            (destination-register-number (ldb (byte 5 7) instruction))
            (destination-register
              (integer-register destination-register-number)))
       (disassemble-integer-register-immediate-instruction
    ...
    (#b0110111
     (let ((destination-register (ldb (byte 5 7) instruction)))
       (disassemble-lui-instruction
        (ldb (byte 20 12) instruction) destination-register)))
    (#b0010111
     (let ((destination-register (ldb (byte 5 7) instruction)))
